...
## Phase 17 — Observability readout: Agent Analytics, Sessions & Intents, health (built Tue 22 night; re-run after the freeze)

**Goal:** show how the agent is monitored and improved in production, using the
traffic generated by Phases 12–16.

**Built early, on Tue 22 night**, on v38/v39/v40 traffic: `run01` is in
`docfiles/observability_readout_run01.md`, the scripts are
`sfdx-project/scripts/observability_readout.apex` and `…_session_detail.apex`, and the health alert
exists. **Wed 23 evening is now a re-run, not a build**: re-run both scripts as `run02` on the
frozen version, compare against `run01`, and retake the screenshots. No Apex class was deployed —
the skill's `AgentforceOptimizeService` was deliberately skipped in favour of anonymous Apex over
`ConnectApi.CdpQuery`, so no permission set changed.
...
5. **Fallback if the dashboards stay empty:** the two readout scripts (see above) plus
   `sf agent trace read`, shown as the observability story. They read the same STDM tables the
   dashboards do, so they are also the reconciliation check when the dashboards *do* render.
